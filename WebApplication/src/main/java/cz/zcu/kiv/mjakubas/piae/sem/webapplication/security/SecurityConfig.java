...
public class SecurityConfig {
    ...
    public void configureAuthentication(AuthenticationManagerBuilder auth) throws Exception {
        var sqlUsers = "SELECT e.emp_orion_login as username, password, enabled FROM user " +
                "INNER JOIN employee e ON e.employee_id=us_employee " +
                "WHERE e.emp_orion_login=?;";
        var sqlRoles = "SELECT e.emp_orion_login as username, a.auth_authotity FROM user " +
                "INNER JOIN employee e ON e.employee_id=us_employee " +
